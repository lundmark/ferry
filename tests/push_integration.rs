...
fn push_uploads_new_and_local_changed_files() {
    ...
    ftp.upload_bytes(&remote_path("keep.txt"), keep_bytes)
        .unwrap();
    ftp.upload_bytes(&remote_path("update.txt"), update_remote_original)
        .unwrap();
    ...
    let update_remote_after = ftp.download(&remote_path("update.txt")).unwrap();
    assert_eq!(
    ...
    let newfile_remote_after = ftp.download(&remote_path("newfile.txt")).unwrap();
    assert_eq!(
    ...
    let keep_remote_after = ftp.download(&remote_path("keep.txt")).unwrap();
    assert_eq!(
...
fn push_refuses_remote_changed_without_force_and_obeys_force() {
    ...
    let still_remote = ftp.download(&remote_path("edited.txt")).unwrap();
    assert_eq!(
    ...
    let after_force = ftp.download(&remote_path("edited.txt")).unwrap();
